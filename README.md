# JavaScript & TypeScript Learning Journey

Learning JS/TS for React Native mobile development at Boost Bank.

## Progress Tracker

### Week 1: JavaScript Fundamentals
- [x] Day 1 - Variables & Data Types
- [x] Day 2 - Functions & Scope
- [x] Day 3 - Arrays & Array Methods
- [x] Day 4 - Objects & Object Methods
- [x] Day 5 - Arrow Functions & This
- [x] Day 6 - Destructuring
- [x] Day 7 - Spread & Rest Operators

### Week 2: Advanced JS
- [X] Day 8 - Promises & Async/Await
- [X] Day 9 - Array Methods (map, filter, reduce)
- [X] Day 10 - ES6 Modules
- [X] Day 11 - Error Handling
- [X] Day 12 - Closures
- [X] Day 13 - Callbacks
- [X] Day 14 - Event Loop

### Week 3: TypeScript Basics
- [X] Day 15 - Types & Interfaces
- [X] Day 16 - Functions in TS
- [ ] Day 17 - Generics
- [ ] Day 18 - Unions & Intersections
- [ ] Day 19 - Type Guards
- [ ] Day 20 - Utility Types
- [ ] Day 21 - Enums

### Week 4: React Native Patterns
- [ ] Day 22 - useState with TS
- [ ] Day 23 - useEffect patterns
- [ ] Day 24 - Custom Hooks
- [ ] Day 25 - Navigation Types
- [ ] Day 26 - API Integration
- [ ] Day 27 - Props & Components
- [ ] Day 28 - Context API

## Daily Learning Template

Each file includes:
1. Concept explanation
2. Code examples
3. Real React Native use cases
4. Practice exercises

## How to Run
```bash
# JavaScript files
npm run run:js 01-javascript-basics/day1-variables.js

# TypeScript files
npm run run:ts 03-typescript-basics/day9-types.ts
```